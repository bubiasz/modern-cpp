//
//  test_stdalgorithms.cpp
//  lab09
//
//  Created by Jakub Matysek on 06/05/2024.
//

#include "../stdalgorithms.cpp"
#include <algorithm>
#include <iostream>
#include <iterator>
#include <numeric>
...
void isAnagramTest(
    const std::string& s1, const std::string& s2, bool expected) {
...
int main() {
    ...
    // Fill vector v with consecutive numbers starting with -5. (Hint: use
    // std::iota) (1 line)
    std::iota(v.begin(), v.end(), -5);
    print(v);

    std::vector<int> odd;
    // Copy to odd all odd numbers from v (Hint: use copy_if and
    // back_inserter) (1 line)
    std::copy_if(v.begin(), v.end(), std::back_inserter(odd),
        [](int x) { return x % 2 != 0; });
    print(odd);

    // Each number x in v replace with x*x-1 (Hint std::transform) (1 line)
    std::transform(
        v.begin(), v.end(), v.begin(), [](int x) { return x * x - 1; });
    print(v);
    ...
    std::set_intersection(v.begin(), v.end(), odd.begin(), odd.end(),
