...
int main(int argc, char* argv[]) {
    ...
    // CODE REPLACED BY MULTITHREADED VERSION BELOW
    // for(unsigned y=0; y<imageHeight; y++){
    //     for(unsigned x=0; x<imageWidth; x++){

    //         Complex c = pixelToComplex(x, y);

    //         auto numberOfIteration = MandelbrotSetIterations(c,
    //         maxNumberOfIteration, escapeThreshold); auto pixel =
    //         iterationsToPixel(numberOfIteration); Output.SetPixel(x,y,pixel)
    //         ;
    //     }
    // }
    ...
    std::cout << "Working on: " << height_chunks << "*" << width_chunks
              << "=" << height_chunks * width_chunks << " threads." << std::endl;

    auto computeChunk = [&](int x_s, int x_e, int y_s, int y_e) {
        for (unsigned y = y_s; y < y_e; y++) {
            for (unsigned x = x_s; x < x_e; x++) {
                Complex c = pixelToComplex(x, y);
                auto numberOfIteration = MandelbrotSetIterations(
                    c, maxNumberOfIteration, escapeThreshold);
    ...
    createThread(
        (height_chunks - 1) * (imageHeight / height_chunks), imageHeight);
