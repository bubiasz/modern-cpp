...
void PixelToPixelCopy(
    BMP& From, int FromX, int FromY, BMP& To, int ToX, int ToY) {
...
void RangedPixelToPixelCopy(BMP& From, int FromL, int FromR, int FromB,
    ...
    for (j = FromT; j <= FromB; j++) {
        for (i = FromL; i <= FromR; i++) {
            PixelToPixelCopy(
                From, i, j, To, ToX + (i - FromL), ToY + (j - FromT));
        }
    }
...
void RangedPixelToPixelCopyTransparent(BMP& From, int FromL, int FromR,
    int FromB, int FromT, BMP& To, int ToX, int ToY, RGBApixel& Transparent) {
    // make sure the conventions are followed
    if (FromB < FromT) {
        int Temp = FromT;
        FromT = FromB;
